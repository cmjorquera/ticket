...
class Logica
{
    ...
    public function MensajesUsuarios($emisor, $receptor)
    {
        $db = new MySQL("", "", "");

        $sql = "SELECT id, mensaje, de, para, fecha, hora,tiempo
                FROM mensajes
                WHERE ((de = '$emisor' AND para = '$receptor') OR (de = '$receptor' AND para = '$emisor'))
                ORDER BY hora ASC";
