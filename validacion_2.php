...
if (empty($usu)) {
    header("Location: login_2.php?error=usuario_vacio");
    exit;
}

if (empty($pass)) {
    header("Location: login_2.php?error=clave_vacio");
    exit;
}

// Validar que el email tenga un formato correcto
if (!filter_var($usu, FILTER_VALIDATE_EMAIL)) {
    header("Location: login_2.php?error=formato_incorrecto");
    exit;
}
...
if ($db->num_rows($consulta) == 1) {
    ...
    if ($usuario['estado'] === 'bloqueado' || $usuario['intentos_fallidos'] >= 3) {
        header("Location: login_2.php?error=bloqueado");
        exit;
    }
    ...
    if (password_verify($pass, $usuario['clave'])) {
        ...
        if ($nuevosIntentos < 3) {
            header("Location: login_2.php?error=credenciales_incorrectas&email=" . urlencode($usu));
            exit;
        } else {
            ...
            header("Location: login_2.php?error=bloqueado&email=" . urlencode($usu));
            exit;
        }
    }
} else {
    header("Location: login_2.php?error=usuario_no_existe");
    exit;
}
?>