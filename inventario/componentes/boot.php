...
register_shutdown_function(function () {
    ...
    @file_put_contents(dirname(__DIR__) . '/error_inventario.log', $log, FILE_APPEND);
});

function inventario_responder_error($mensaje, $codigo = 500)
{
    http_response_code((int)$codigo);
    $esAjax = strpos(str_replace('\\', '/', $_SERVER['SCRIPT_NAME'] ?? ''), '/inventario/ajax/') !== false;
    ...
    echo '<!DOCTYPE html><html lang="es"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Error inventario</title>';
    ...
    echo '</head><body><div class="box"><h1>Inventario no pudo cargar</h1><p>' . htmlspecialchars((string)$mensaje, ENT_QUOTES, 'UTF-8') . '</p></div></body></html>';
    exit;
}
...
try {
    ...
    inventario_responder_error('Error al iniciar el modulo de inventario: ' . $e->getMessage());
}
...
function inventario_url($ruta = '')
{
    ...
    $pos = strpos($scriptName, '/inventario');
    if ($pos !== false) {
