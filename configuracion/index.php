...
$esSuperAdmin = false;
try {
    $esSuperAdmin = (bool) $db->fetchOne(
        'SELECT 1 FROM usuario_perfil WHERE id_usuario = ? AND id_perfil = 3 LIMIT 1',
        [(int) Sesion::get('id', 0)]
    );
} catch (Throwable $ex) {
    error_log('No fue posible validar el acceso al organigrama: ' . $ex->getMessage());
}
...
<div class="quick-grid">
  ...
  <?php if ($esSuperAdmin): ?><a class="quick-card" href="organigrama_colegios.php"><div class="top"><strong>Organigrama de Colegios</strong></div><p>Responsables, departamentos y usuarios</p><small>Consultar estructura institucional</small></a><?php endif; ?>
