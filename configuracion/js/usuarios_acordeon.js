(() => {
  ...
  function createEmptyDepartment(department) {
    ...
    section.dataset.departmentOrder = String(department.orden || 0);
    section.innerHTML = `
  ...
  async function loadSchoolIcon(holder) {
    ...
    try {
      ...
      if (data.existe && data.icono) {
        ...
        holder.textContent = data.iniciales || holder.dataset.fallback || 'C';
      }
    } catch (_) {
  ...
  async function reloadSchoolDepartments(schoolId) {
    ...
    departments.forEach(department => {
      ...
      group.dataset.departmentOrder = String(department.orden || 0);
  ...
  function abrirModalAgregarDepto(idColegio, nombreColegio = 'Colegio') {
    ...
    document.getElementById('departamento-orden').value = '';
  ...
  async function saveDepartment() {
    ...
    const orden = Number(document.getElementById('departamento-orden').value || 0);
    error.hidden = true;

    if (!idColegio || !nombre || !/^[A-ZÁÉÍÓÚÑ0-9]{2,3}$/u.test(sigla) || orden <= 0) {
      error.textContent = 'Completa el nombre, una sigla de 2 a 3 caracteres y un orden mayor que cero.';
      error.hidden = false;
      return;
    }
    ...
    try {
      ...
      if (window.confirm('Departamento creado. ¿Recargar vista?')) {
