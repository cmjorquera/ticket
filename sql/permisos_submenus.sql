-- Ejecutar una sola vez en crist668_sistema_panel_central.
-- Agrega permisos por submenú y conserva el comportamiento actual:
-- cada permiso de menú existente recibe inicialmente todos sus submenús.
...
SELECT DISTINCT pm.id_usuario, pm.id_menu1, sm.id_submenu, pm.id_tipo_permiso
 ...
 WHERE pm.id_submenu IS NULL
   ...
   AND NOT EXISTS (
       SELECT 1
         FROM permisos_menu_1 existente
