    ...
    function initTablaSoftware() {
        ...
        tablaSoftware = $tabla.DataTable({
            ...
            columns: [
                ...
                { data: null, className: 'celda-opciones', orderable: false, searchable: false, render: row => `<div class="inv-action-buttons d-flex flex-nowrap align-items-center justify-content-start gap-2"><a href="ver_software.php?id_software=${parseInt(row.id_software, 10)}" class="btn btn-secondary inv-system-icon-btn" title="Ver software"><i class="bi bi-eye"></i></a><a href="editar_software.php?id_software=${parseInt(row.id_software, 10)}" class="btn btn-primary inv-system-icon-btn" title="Editar software"><i class="bi bi-pencil"></i></a><button type="button" class="btn btn-danger inv-system-icon-btn btnEliminarSoftware" data-id="${parseInt(row.id_software, 10)}" title="Eliminar software"><i class="bi bi-trash"></i></button></div>` }
            ]
        });
        loadSoftware();
    }
    ...
    function initTablaSitios() {
        ...
        tablaSitios = $tabla.DataTable({
            ...
            columns: [
                ...
                { data: 'responsable', className: 'celda-tecnico', render: data => escapeHtml(data || 'Sin asignar') },
                { data: null, className: 'celda-opciones', orderable: false, searchable: false, render: row => `<div class="inv-action-buttons d-flex flex-nowrap align-items-center justify-content-start gap-2"><a href="ver_sitio_web.php?id_sitio=${parseInt(row.id_sitio, 10)}" class="btn btn-secondary inv-system-icon-btn" title="Ver sitio"><i class="bi bi-eye"></i></a><a href="editar_sitio_web.php?id_sitio=${parseInt(row.id_sitio, 10)}" class="btn btn-primary inv-system-icon-btn" title="Editar sitio"><i class="bi bi-pencil"></i></a><button type="button" class="btn btn-danger inv-system-icon-btn btnEliminarSitio" data-id="${parseInt(row.id_sitio, 10)}" title="Eliminar sitio"><i class="bi bi-trash"></i></button></div>` }
            ]
        });
        loadSitios();
    }
